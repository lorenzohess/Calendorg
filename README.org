* Calendorg
Highly opinionated bi-weekly schedule viewer and time budgeter for Doom Emacs.
...
*Features:*
1. Add permanent commitments into your schedule.
2. Allocate timeblocks to commitments.
3. See if you meet your weekly hour targets for any hourly commitments.
4. Beautiful SVG rendering.
...
Both files live in =calendorg-schedule-dir=, which defaults to the =SCHEDULE_DIR= environment variable, or =~/nextcloud-sync/todo/=. Write =schedule.org= there following [[file:schedule-format.org][schedule-format.org]]. Then =calendorg-blocks.org= is created on first allocation. And =calendorg-log.org= maintains a log.
** Usage
...
| =i=          | edit selected                             |
| =x=          | delete allocation or skip scheduled block |
| =c=          | comment on selected                       |
| =gr=         | reload from disk                          |
| =w=          | hide/show Week 2                          |
| =[=          | visualize previous week(s)                |
| =]=          | visualize next week(s)                    |
| =q=          | quit                                      |
...
| =calendorg-log-file=      | =calendorg-log.org=                          | Calendorg log of past weeks, and carry-over time. |
